...
function buildLast30(dailyHistory: DailyHistory, goalMl: number, consumedMl: number) {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    if (i === 0) {
      days.push({ date: key, label: `${dd}/${mm}`, consumed: consumedMl, goal: goalMl })
    } else {
      const entry = dailyHistory[key]
      days.push({ date: key, label: `${dd}/${mm}`, consumed: entry?.consumed ?? 0, goal: entry?.goal ?? goalMl })
    }
  }
  return days
}
...
export function HistoryModal({ dailyHistory, goalMl, consumedMl, streak, bestStreak, onClose }: Props) {
  ...
  const avgMl     = knownDays > 0 ? Math.round(days.reduce((s, d) => s + d.consumed, 0) / knownDays) : 0
  ...
  const barColor = (d: (typeof days)[0]) => {
    ...
    return d.consumed >= d.goal ? "#22c55e" : "#f97316"
  }
    ...
    >
      ...
      >
        ...
        <div className="px-5">
          <div className="overflow-x-auto rounded-2xl border border-border bg-background p-3">
            <svg width={W} height={H + 28} style={{ display: "block" }}>
              <line x1={0} y1={goalY} x2={W} y2={goalY} stroke="#3b82f6" strokeWidth={1} strokeDasharray="4 3" opacity={0.5} />
              {days.map((d, i) => {
                const barH = Math.max(3, (d.consumed / maxVal) * H)
                const y = H - barH, x = barX(i)
                const isToday = d.date === todayKey
                return (
                  <g key={d.date}>
                    <rect x={x} y={0} width={barW} height={H} rx={3} fill="transparent" />
                    <rect x={x} y={y} width={barW} height={barH} rx={3} fill={barColor(d)} opacity={isToday ? 1 : 0.85} />
                    {isToday && <rect x={x - 1} y={y - 1} width={barW + 2} height={barH + 2} rx={4} fill="none" stroke="#fff" strokeWidth={1} opacity={0.4} />}
                  </g>
                )
              })}
              {days.map((d, i) => {
                if (i !== 0 && (i + 1) % 5 !== 0) return null
                return <text key={d.date} x={barX(i) + barW / 2} y={H + 18} textAnchor="middle" fontSize={9} fill="#6b7280">{d.label}</text>
              })}
            </svg>
          </div>
        </div>
        ...
        <div className="mt-3 flex items-center justify-center gap-4 px-5 text-xs text-muted-foreground">
          ...
          <LegendDot color="#3b82f6" dashed label="Meta" />
        </div>
      </div>
    </div>
  )
...
function LegendDot({ color, label, dashed }: { color: string; label: string; border?: boolean; dashed?: boolean }) {
  if (dashed) return (
